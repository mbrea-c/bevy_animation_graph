...
impl<'de> DeserializeSeed<'de> for AnimationNodeLoadDeserializer<'_, '_> {
        ...
        D: serde::Deserializer<'de>,
    {
                ...
                D: Deserializer<'de>,
            {
                let mut processor = ValueProcessor {
                    ...
                    deserialize: Box::new(|registration, deserializer| {
                        deserialize_handle(registration, deserializer, self.load_context)
                    }),
                };
                let value = ReflectDeserializer::new(self.type_registry, Some(&mut processor))
                ...
                let reflect_target = self
                    .type_registry
                    .get_type_data::<ReflectNodeLike>(type_id)
                    .ok_or_else(|| {
                        de::Error::custom(format!("`{type_path}` does not `#[reflect(NodeLike)]`"))
                    })?;
                let target = reflect_target
                    .get_boxed(value)
                    .expect("should be able to downcast value into `NodeLike`");
        ...
        impl<'de> Visitor<'de> for ValueVisitor<'_, '_> {
            ...
            {
                const INVALID_LENGTH: &str = "struct AnimationNode with 2 elements";
